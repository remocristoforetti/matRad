function [expDist, mAlphaDoseExp, mSqrtBetaDoseExp, mAlphaDoseOmega, mSqrtBetDoseOmega, mAlphaSqrtBetaDoseOmega,mTwiceAlphaSqrtBetaDoseOmega,alphaJExp, sqrtBetaJExp, probQuantitiesAccumulationTime] = matRad_accumulateBioProbabilisticQuantitiesExtended(saveDir,ct,cst,scenariosMeta, multiScen, mode4D)
    ...
    for phaseIdx=1:numel(ctAccumIx)
        ...
        currPhaseOmegaAlphaBeta      = mAlphaSqrtBetaDoseOmega(:,phaseIdx);
        ...
        for scenIdx=1:nScenariosInPhase
            ...
            [scenarioDistribution,scenarioAlphaDose,scenarioSqrtBetaDose,~, scenarioAlphaJ, scenarioSqrtBetaJ] = matRad_loadScenariosFromMeta(saveDir,currMeta,dijTemplate);
            ...
            for structIdx=structsToInclude
                ...
                % Omega AlphaBeta
                currStructOmegaAlphaBeta = currPhaseOmegaAlphaBeta{structIdx};
                currStructOmegaAlphaBeta = currStructOmegaAlphaBeta + gather(currDistAlpha'*currDistBeta).*scenWeights{phaseIdx}(scenIdx);
                wait(gpu);
                
                currPhaseOmegaAlphaBeta{structIdx} = currStructOmegaAlphaBeta;

                % AlphaJ
                currStructAlphaJ = currPhaseAlphaJ{structIdx};
                currStructAlphaJ = currStructAlphaJ + scenarioAlphaJ{1}{structIdx}'.*scenWeights{phaseIdx}(scenIdx);
                
                currPhaseAlphaJ{structIdx} = currStructAlphaJ;

                % BetaJ
                currStructSqrtBetaJ = currPhaseSqrtBetaJ{structIdx};
                currStructSqrtBetaJ = currStructSqrtBetaJ + scenarioSqrtBetaJ{1}{structIdx}'*scenWeights{phaseIdx}(scenIdx);
        ...
        mTwiceAlphaSqrtBetaDoseOmega(:,phaseIdx) = currPhaseTwiceAlphaSqrtBetaDoseOmega;
        alphaJExp(:, phaseIdx)              = currPhaseAlphaJ;
        sqrtBetaJExp(:,phaseIdx)               = currPhaseSqrtBetaJ;
