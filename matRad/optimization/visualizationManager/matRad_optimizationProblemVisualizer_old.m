    ...
    methods
        function this = matRad_optimizationProblemVisualizer_old(cst, distributionProperties)
            ...
            if ~isempty(cst)
                ...
                if ~isfield(distributionProperties, 'plane')
                    distributionProperties.plane = 3;
                end

                if ~isfield(distributionProperties, 'slice')
                    distributionProperties.slice = floor(distributionProperties.ct.cubeDim(3)/2);
                end
                this.distibutionProperties = distributionProperties;

            end
        end

        function updateData(this, objFunctionValues, totF, d)
        ...
        function plotFunction(this)
            ...
            x = [1:this.iter];
                if ~this.isOpen
                    ...
                    curr_hFig = figure('Name','Visualization Manager','NumberTitle','off', 'WindowState','maximized');
                    ...
                    matRad_plotSliceWrapper(this.hAx_distribution, this.distibutionProperties.ct, this.distibutionProperties.cst, 1, dCtGrid, this.distibutionProperties.plane, this.distibutionProperties.slice, [],[],[],[],[],[],ones(1,size(this.distibutionProperties.cst,1)));
