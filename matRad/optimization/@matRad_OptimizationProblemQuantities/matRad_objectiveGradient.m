...
for modalityIdx=1:optiProb.BP.nModalities
    modalityName = optiProb.BP.radiationModalities{modalityIdx};
...
for qtIdx=optiProb.BP.optimizationQuantities
    ...
    quantityNames = cellfun(@(x) x.quantityName, optiProb.BP.quantities, 'UniformOutput',false);
    quantityInstance = optiProb.BP.quantities{strcmp(qtIdx{1},quantityNames)};
    
    for elementIdx=nScensOrStructs
        if isprop(quantityInstance, 'modality')
            currQuantityModality = quantityInstance.modality;
            
            % SPlit the weights to see how many are there
            ...
            weightGradient(splitW.([currQuantityModality, '_idx'])) = weightGradient(splitW.([currQuantityModality, '_idx'])) + g.(qtIdx{1}){elementIdx};
