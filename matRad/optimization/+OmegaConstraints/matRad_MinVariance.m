classdef matRad_MinVariance < OmegaConstraints.matRad_VarianceConstraint
    ...
    properties (Constant)
        name = 'Min Varaince constraint';
        parameterNames = {'var^{min}'};
        parameterTypes = {'minVariance'};
    end
...
%         quantity;
    ...
    methods
        function this = matRad_MinVariance(minVaraince)
            if exist('minVaraince', 'var') && ~isempty(minVaraince)
                this.parameters = {minVaraince};
            end
        end
        ...
        function cVarianceJacob  = computeVarianceConstraintJacobian(~,dOmega, ~)
            if ~isscalar(dOmega)
                ...
                %cVarianceJacob = (2/nVoxels) * dOmega;
                cVarianceJacob = dOmega;
            else
                %cVarianceJacob = (2/nVoxels);
                cVarianceJacob = 1;
