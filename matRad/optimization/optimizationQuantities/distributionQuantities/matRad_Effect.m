classdef matRad_Effect < matRad_DistributionQuantity
    ...
    methods
        ...
        % function gradientOutput = computeQuantityGradient(this,dij,scen,w)
        ...
        % 
        %     alphaGrad = this.subQuantities{1}.getQuantityGradient(dij,w);
        %     betaDose  = this.subQuantities{2}.getResult(dij,w);
        %     betaGrad  = this.subQuantities{2}.getQuantityGradient(dij,w);
        % 
        %     gradientOutput = alphaGrad{scen} + 2 * betaDose{scen} .* betaGrad{scen};
        % end
