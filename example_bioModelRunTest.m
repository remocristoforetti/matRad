...
modelNamesPhotons = {'none', 'constRBE'};
...
for qtOpt = quantitiesOptimization
    for model = modelNamesPhotons
        ...
        pln.bioParam = matRad_bioModel(pln.radiationMode,qtOpt{1},model{1},pln.machine);

        currentTable.model = pln.bioParam.model;
        currentTable.quantityOpt = pln.bioParam.quantityOpt;
        currentTable.quantityVis = pln.bioParam.quantityVis;
        currentTable.bioOpt = pln.bioParam.bioOpt;
...
outcomeTable = table('Size', [length(outcomeStruct) 5], 'VariableTypes', {'string', 'string','string', 'logical', 'logical'});
outcomeTable.Properties.VariableNames = {'model', 'quantityOpt','quantityVis','bioOpt', 'optimized'};
...
outcomeTable.quantityOpt         = {outcomeStruct.quantityOpt}';
outcomeTable.quantityVis         = {outcomeStruct.quantityVis}';
outcomeTable.bioOpt              = {outcomeStruct.bioOpt}';
