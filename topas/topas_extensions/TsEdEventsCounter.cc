// Scorer for TsEdEventsCounter
#include "TsEdEventsCounter.hh"

#include "G4SystemOfUnits.hh"

TsEdEventsCounter::TsEdEventsCounter(TsParameterManager* pM, TsMaterialManager* mM, TsGeometryManager* gM, TsScoringManager* scM, TsExtensionManager* eM,
...
TsEdEventsCounter::~TsEdEventsCounter()
{
	free (fEBinning);
	free (fEdepBinning);
...
G4bool TsEdEventsCounter::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
	...
	if (particleAtomicMass > 0 && eDep >0)
	{
		...
		// multiply weight of particle times the eDep
		Weight *= eDep;
...
G4int TsEdEventsCounter::interpolateIndex(G4double Value, G4double* Binning, G4int numberOfBins)
{
	G4int i = 0;
...
G4bool TsEdEventsCounter::GetBinning(G4double BinMax, G4double BinMin, G4int numOfBins, G4double* fBinning)
{
