function writeEventsCounterScorers(EParam, Ions, topas_cfg, phantom, currDir)
   ...
   for k=1:nIons
      fileName = [currDir, filesep,'Phi_',phantom.name, '_', Ions{k}, '.txt'];
      fID = fopen(fileName, 'w');
      
      fprintf(fID,'#-- Event counter binned by energy Scorer for %s, this scores the number of events within the voxel \n',Ions{k});
      %fprintf(fID,'s:Sc/%s/%s/%s/Quantity = "TsEdEventSpectrum" \n',phantom.name,'SPeD',Ions{k});
      %fprintf(fID,'s:Sc/%s/%s/%s/Quantity = "TsDoseEventsScorer" \n',phantom.name,'SPdose',Ions{k});
      fprintf(fID,'s:Sc/%s/%s/%s/Quantity = "TsEventsCounter" \n',phantom.name,'Phi',Ions{k});   
      fprintf(fID,'s:Sc/%s/%s/%s/OutputType = "DICOM" \n',phantom.name,'Phi',Ions{k});
      fprintf(fID,'s:Sc/%s/%s/%s/Component = "%s" \n',phantom.name,'Phi',Ions{k},phantom.name);
      fprintf(fID,'s:Sc/%s/%s/%s/OutputFile = "./Output/Phi/%s/Ion_%1d/Phi_%s" \n',phantom.name,'Phi',Ions{k},phantom.name,IonsZ(k),Ions{k});
      fprintf(fID,'i:Sc/%s/%s/%s/OnlyIncludeParticlesOfAtomicNumber = %1d \n',phantom.name,'Phi',Ions{k},IonsZ(k));
      fprintf(fID,'s:Sc/%s/%s/%s/IfOutputFileAlreadyExists = "Overwrite" \n',phantom.name,'Phi',Ions{k});
      fprintf(fID,'b:Sc/%s/%s/%s/OutputAfterRun = "True" \n',phantom.name,'Phi',Ions{k});
      fprintf(fID, '\n');
      
      fprintf(fID,'d:Sc/%s/%s/%s/Emax = %3.4f MeV \n',phantom.name,'Phi',Ions{k},EParam(k).EMax);
      fprintf(fID,'d:Sc/%s/%s/%s/Emin = %3.4f MeV \n',phantom.name,'Phi',Ions{k},EParam(k).EMin);
      fprintf(fID,'i:Sc/%s/%s/%s/nEBins = %4u \n',phantom.name,'Phi',Ions{k},EParam(k).nEBins);
      fprintf(fID, '\n');
...
%       fprintf(fID, '\n');
      ...
      fprintf(fID,'i:Sc/%s/%s/%s/ZBins = Sc/%s/%s/%s/nEBins + 1 \n',phantom.name,'Phi',Ions{k},phantom.name,'Phi',Ions{k});
