classdef matRad_mixMod_allVsall < matRad_ScenarioModel
%This scenario modlaity combines the nominal photon scenario with all the proton scenarios and the noinal proton scenario with all the photon scenarios    
    ...
    methods
        function obj = matRad_mixMod_allVsall(ct, originalScenarios)
            if nargin == 0 
                superclassArgs = {};
            else
                superclassArgs = {ct};
        ...
        function set.originalScenarios(obj,originalScenarios)
            ...
            obj.nSamples = prod(nSmodality); %-1 here to avoit double counting of nominal-nominal combination    
        ...
        function scenarios = updateScenarios(obj)
            ...
            tmp = repmat([1:obj.originalScenarios{1}.nSamples], obj.originalScenarios{2}.nSamples,1);
            combMask(:,1) = tmp(:);

            tmp = repmat([1:obj.originalScenarios{2}.nSamples], 1, obj.originalScenarios{1}.nSamples)';
            combMask(:,2) = tmp;
