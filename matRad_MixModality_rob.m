...
pln(1).propOpt.STfractions     = pln(1).numOfFractions;             % can also do different spread of the fractions between scenes ( make sure sum(STfractions == numOfFractions)
...
pln(2).propOpt.STscenarios     = 1;
pln(2).propOpt.STfractions     = pln(2).numOfFractions;             % can also do different spread of the fractions between scenes ( make sure sum(STfractions == numOfFractions)
...
%% Visualize sigme modlaity
slice = 59;
qtOpt = plnJO.bioParam.quantityOpt;
modalities = {'photon'};

f = figure;
if any(strcmp(qtOpt, {'RBExD', 'effect'}))
    subplot(1,3,1);
    imagesc(resultGUI{1}.physicalDose(:,:,slice));
    matRad_plotVoiContourSlice(gca(f), cst,ct.cube, 1, 1,3,slice);
    title('physicalDose');
    
    if isfield(resultGUI{1}, 'RBExD')
        subplot(1,3,2);
        imagesc(resultGUI{1}.RBExD(:,:,slice));
        matRad_plotVoiContourSlice(gca(f), cst,ct.cube, 1, 1,3,slice);
        title('RBExD');

    end
    
    if isfield(resultGUI{1}, 'effect')
        subplot(1,3,3);
        imagesc(resultGUI{1}.effect(:,:,slice));
        matRad_plotVoiContourSlice(gca(f), cst,ct.cube, 1, 1,3,slice);
        title('effect');
    end
else
