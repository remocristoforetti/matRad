    ...
    methods
        ...
        function set.baseData(obj,value)
            ...
            if ~isempty(obj.radiationMode)
                 currmachine_name = [obj.radiationMode, '_', currBaseData];
                 machine_name     = [obj.radiationMode, '_', value];
            end
            if ~strcmp(currmachine_name,machine_name) && ~(isempty(currBaseData) && isempty(value))
                if ~isempty(value)
                    if ~isempty(obj.radiationMode) %here if radiationMode is empty, dose nothing -> change
                           ...
                           elseif isempty(obj.RequiredBaseData)
                                matRad_cfg.dispInfo(' done\n');
                                matRad_cfg.dispInfo(['No baseData required for model: ', obj.model, '\n']);
                                obj.baseData = [];
                                obj.updateStatus();
                           end
                    end
       ...
       function updateStatus(obj)
            ...
            % elseif ~isempty(currRadiationMode) && isempty(currBaseData) && isempty(obj.RequiredBaseData)
            %      obj.calcBioParameters = 1;
            %      matRad_cfg.dispInfo('All parameters are correct, enabling biological model calculation.\n');
            else
