...
f_OWC = zeros(size(dij.physicalDose));
...
try
                if isa(robustness, 'Robustness.matRad_Robustness') && ~isa(robustness, 'Robustness.matRad_RobustnessCOWC')
                    
                    % get the dose distributions
                    [d_i, idxs] = robustness.getDi(d,cst{i,4});
                    
                    %Compute the gradients
                    doseGrad_i = cellfun(@objective.computeDoseObjectiveGradient, d_i, 'UniformOutput', false);

                    doseGrad_i = cellfun(@(grad) objective.penalty*grad, doseGrad_i, 'UniformOutput',false);
                    
                    
                    % This is only needed for OWC case
                    fGrad = [];
                    if isa(robustness, 'Robustness.matRad_RobustnessOWC')
                        f_i = cellfun(@objective.computeDoseObjectiveFunction, d_i, 'UniformOutput', false);
                        f_i = cellfun(@(objFunc) objective.penalty.*objFunc, f_i);
                        
                        switch optiProb.useMaxApprox
                            case 'logsumexp'
                                [~,fGrad] = optiProb.logSumExp(f_i);
                            case 'pnorm'
                                [~,fGrad] = optiProb.pNorm(f_i,numel(robstness.useScen));
                            case 'none'
                                [~,ix] = max([f_i{:}]);
                                fGrad = zeros(size(f_i));
                                fGrad(ix) = 1;
                            case 'otherwise'
                        ...
                        robustness.updatefGrad(fGrad);
                    end
                
                    doseGradient = robustness.getRobustGradient(doseGradient,doseGrad_i,idxs);


                elseif isa(robustness, 'Robustness.matRad_RobustnessCoWC')
                    matRad_cfg.dispError('COWC not yet implemented');
                end

catch
                % switch robustness.name
