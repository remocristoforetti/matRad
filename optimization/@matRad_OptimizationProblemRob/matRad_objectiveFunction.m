        ...
        for j = 1:numel(cst{i,6})
            ...
            if isa(objective,'DoseObjectives.matRad_DoseObjective')
                ...
                if isa(robustness, 'Robustness.matRad_Robustness') && ~isa(robustness, 'Robustness.matRad_RobustnessCOWC')
                    [d_i] = robustness.getDi(d,cst{i,4});
                    ...
                    if isa(robustness, 'Robustness.matRad_RobustnessOWC')
                        switch optiProb.useMaxApprox
                            case 'logsumexp'
                                f_i = optiProb.logSumExp(f_i);
                            case 'pnorm'
                                f_i = optiProb.pNorm(f_i,numel(robustness.useScen));
                            case 'none'
                                f_i = max(f_i);
                            case 'otherwise'
...
if f_i > 0
    switch optiProb.useMaxApprox
        case 'logsumexp'
            f_i = optiProb.logSumExp(f_COWC);
        case 'pnorm'
            f_i = optiProb.pNorm(f_COWC,numel(useScen));
        case 'none'
            f_i = max(f_COWC);
        case 'otherwise'
            matRad_cfg.dispWarning('Unknown maximum approximation desired. Using ''none'' instead.');
