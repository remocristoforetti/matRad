...
%{
            ...
            elseif isa(optiProb.BP,'matRad_EffectProjection') && ~isempty(jacobSub)
               ...
               %if isa(optiProb.BP,'matRad_VariableRBEProjection')
                        ...
                        %     % currently for ST optimization 
                        %     dt = sum(dt{scen},2);
                        %     sqrt_mod = sqrt_mod + dt(cst{i,4}{1}) + dij.original_Dijs{1}.gamma(cst{i,4}{1});
                        % 
                        % end
                        % 
                  ...
                  %for mod = 1:length(dij.original_Dijs)
                    % for the time being, no scenarios here
                   % scaledEffect = (dij.original_Dijs{1}.gamma(cst{i,4}{1}) + d_singleModality{mod,1}(cst{i,4}{1}));
                    %scaledEffect = sqrt_mod;
                    %jacobSub_mod(mod)     = {jacobSub./(2*dij.original_Dijs{1}.bx(cst{i,4}{1}) .* scaledEffect)};
                  %end
               ...
               mSqrtBetaDoseProjection{1} =  [mSqrtBetaDoseProjection{1}, sparse(repmat(cst{i,4}{1},nConst,1),repmat(1:numel(cst{i,4}{1}),1,nConst),2*reshape(jacobSub',[],1),dij.doseGrid.numOfVoxels,nConst*numel(cst{i,4}{1}))];
                %mSqrtBetaDoseProjection{1} =  [mSqrtBetaDoseProjection{1}, sparse(repmat(cst{i,4}{1},nConst,1),repmat(1:numel(cst{i,4}{1}),1,nConst),reshape(jacobSub',[],1),dij.doseGrid.numOfVoxels,nConst*numel(cst{i,4}{1}))];
