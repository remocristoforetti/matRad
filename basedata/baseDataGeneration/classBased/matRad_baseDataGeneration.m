...
methods
    ...
    function obj = matRad_baseDataGeneration()
            ...
            obj.MCparams = struct('runDirectory', '', ...
                                  'nRuns', 0, ...
                                  'previousRuns', 0, ...
                                  'doubleSource', false, ...
                                  'nPrimaries', 0, ...
        ...
        function updateEnergyParams(obj)
            ...
            if ~isempty(obj.energyParams.EsLUT)
                obj.energyParams.simulateEnergySpread = interp1(obj.energyParams.EsLUT.energies,obj.energyParams.EsLUT.energySpread, obj.simulateEnergies);
            else
                matRad_cfg.dispError('Cannot find energy spread LUT'),
            end
        ...
        function retriveMainClass(obj, fileName)
            ...
            if exist('saveStr', 'var')
                obj.energyParams =  saveStr.energyParams;

                obj.simulateEnergies = saveStr.simulateEnergies;
                if isfield(saveStr, 'scorers')
                ...
                obj.workingDir = saveStr.workingDir;
            end
        end

        function machine = generateMachineFile(obj,doseFit,airWideningFit,spectraFit, protonLETFit)
            ...
            %if size(doseFit,2) == size(airWideningFit,1) && size(doseFit,2) == size(spectraFit.fitSpectraOutput,2)
            if exist('spectraFit', 'var') && exist('protonLETFit', 'var')
                
                [doseFit, spectraFit, protonLETFit] = obj.cutData(doseFit, spectraFit, protonLETFit);
            end
                ...
                for energyIdx = 1:obj.energyParams.nEnergies
                    ...
                    if exist('spectraFit', 'var')
                        ...
                        machine_data(energyIdx).Fluence.Data         = spectraFit.fitSpectraOutput(energyIdx).Fluence;
 
                        machine_data(energyIdx).EnergyDeposit.energies     = spectraFit.fitParams(energyIdx).energies;
                        machine_data(energyIdx).EnergyDeposit.Data         = spectraFit.fitSpectraOutput(energyIdx).EnergyDeposit;
 
                    end
                    if exist('protonLETFit', 'var')
                        machine_data(energyIdx).LET                        = protonLETFit.fitLETOutput(energyIdx).LET'; 
                    end
            ...
            % else
            %     matRad_cfg.dispError('Wrong number of enery inputs');
            % end
        end
        ...
        function [doseFit, spectraFit, protonLETFit] = cutData(obj,doseFit, spectraFit, protonLETFit)
            %Determine the cut in depth according to fluence
            
            if isfield(spectraFit.fitSpectraOutput, 'cumulativeFluence')
                for energyIdx=1:size(doseFit,2)
                    currentFluence = spectraFit.fitSpectraOutput(energyIdx).cumulativeFluence;
                    maxDepthIdx = find(currentFluence < obj.machineParameters.fluenceThreshold*max(currentFluence),1, 'first');
                    if isempty(maxDepthIdx)
                        maxDepthIdx = size(currentFluence,2);
                    end
                    currentDoseFit = doseFit(energyIdx);
                    
                    doseFit(energyIdx).depths  = currentDoseFit.depths(1:maxDepthIdx);
                    
                    if obj.machineParameters.smoothSigmas
                        doseFit(energyIdx).sigma   = smooth(currentDoseFit.sigma(1:maxDepthIdx));
                        doseFit(energyIdx).sigma1  = smooth(currentDoseFit.sigma1(1:maxDepthIdx));
                        doseFit(energyIdx).sigma2  = smooth(currentDoseFit.sigma2(1:maxDepthIdx));
                        doseFit(energyIdx).weight  = smooth(currentDoseFit.weight(1:maxDepthIdx));
                        doseFit(energyIdx).PDD     = smooth(currentDoseFit.PDD(1:maxDepthIdx));
                    else
                        doseFit(energyIdx).sigma   = currentDoseFit.sigma(1:maxDepthIdx);
                        doseFit(energyIdx).sigma1  = currentDoseFit.sigma1(1:maxDepthIdx);
                        doseFit(energyIdx).sigma2  = currentDoseFit.sigma2(1:maxDepthIdx);
                        doseFit(energyIdx).weight  = currentDoseFit.weight(1:maxDepthIdx);
                        doseFit(energyIdx).PDD     = currentDoseFit.PDD(1:maxDepthIdx);
                    end
                    spectraFit.fitSpectraOutput(energyIdx).Fluence       =  spectraFit.fitSpectraOutput(energyIdx).Fluence(:,1:maxDepthIdx);
                    spectraFit.fitSpectraOutput(energyIdx).EnergyDeposit =  spectraFit.fitSpectraOutput(energyIdx).EnergyDeposit(:,1:maxDepthIdx);
                    protonLETFit.fitLETOutput(energyIdx).LET             =  protonLETFit.fitLETOutput(energyIdx).LET(1:maxDepthIdx);
                end
            end
        end
        %% Setters
